...
func TestCosmosDeriveAndSign(t *testing.T) {
	...
	s := New(v)
	compressed, err := s.ExportCompressedPubKey("proj/mantra/alice")
	...
	sig, pk, err := s.SignDirect("proj/mantra/alice", doc)
	if err != nil || len(sig) != 64 || len(pk) != 33 {
		t.Fatalf("direct sig=%d pk=%d err=%v", len(sig), len(pk), err)
	}
	sig, pk, err = s.SignAmino("proj/mantra/alice", []byte(`{"b":2, "a":1}`+"\n"))
	if err != nil || len(sig) != 64 || len(pk) != 33 {
		t.Fatalf("amino sig=%d pk=%d err=%v", len(sig), len(pk), err)
	}
	if _, _, err := s.SignDirect("proj/mantra/alice", []byte("bad")); err == nil || err.Error() != "invalid SignDoc proto encoding" {
