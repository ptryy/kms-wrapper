## Context
The KMS wrapper exposes a single `secp256k1` key per `kms-vault-plugin` key path. The signer layer (`internal/signer/evm`, `internal/signer/cosmos`) is what decides whether a given signing request produces an EVM signature, an EIP-712 signature, a Cosmos DIRECT signature, or an Amino JSON signature. The key path itself is opaque to the signer — both `/sign/evm` and `/sign/cosmos` accept any `key_path` and call into the plugin's `kms/sign/<path>` endpoint with a pre-hashed 32-byte `input` (no `hash_algorithm` parameter; the plugin signs the raw digest). Nothing today binds a path to a chain.
